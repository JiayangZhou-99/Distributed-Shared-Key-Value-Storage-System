...
func TestSyncTwoClientsSameFileLeaderFailure(t *testing.T) {
	...
	cur,_ := test.Clients[0].GetInternalState(test.Context, &emptypb.Empty{})
	println("whether 0 is leader: ",cur.IsLeader)
	...
	println("2")
	cur,_ = test.Clients[1].GetInternalState(test.Context, &emptypb.Empty{})
	println("whether 1 is leader: ",cur.IsLeader)
